...
export function HeroSection({ stats, transactions, trend, onViewInsights }) {
  ...
  return (
    <section className="relative overflow-hidden">
      ...
      <div className="grid grid-cols-1 items-center gap-10 xl:grid-cols-[minmax(0,1.25fr)_minmax(0,1fr)] xl:gap-8">
        {/* ── Copy ────────────────────────────────────────────────────────── */}
        <div className="animate-fade-up">
          ...
          <h1 className="headline mt-4 text-[2.6rem] sm:text-[3.2rem] lg:text-[3.5rem] 2xl:text-[3.85rem] text-ink">
            Your Money.
            <br />
            <span className="text-accent">Under Control.</span>
          </h1>

          <p className="mt-5 max-w-md text-[14.5px] leading-relaxed text-ink-dim">
            Manage your personal and business finances in one intelligent platform.
          </p>

          <div className="mt-8 flex flex-wrap items-center gap-3">
            <button type="button" onClick={() => open('transaction')} className="btn-primary btn-lg">
              <Plus size={18} strokeWidth={2.6} />
              Add Transaction
            </button>
            <button type="button" onClick={onViewInsights} className="btn-secondary btn-lg group">
              View Insights
              <ArrowRight
                size={16}
